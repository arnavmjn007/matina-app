...
function ProfileRightPanel({ userProfile, isGuest, onRequireAuth }) {
  const [isModalOpen, setIsModalOpen] = useState(false);

  const handleSuperlike = () => {
    if (isGuest) {
      message.info("Login to send superlikes.");
      onRequireAuth?.();
      return;
    }
    setIsModalOpen(true);
    ...
    <>
      <section className="p-6 rounded-2xl shadow-lg bg-white h-full flex flex-col justify-between">
        <div>
          ...
          <div className="flex flex-wrap gap-2 mb-8">
            {userProfile.interests?.map((interest, index) => (
              <span
                key={index}
                className="bg-pink-100 text-pink-800 text-md font-medium px-3 py-1 rounded-full"
              >
                {interest}
              </span>
            ))}
          ...
          <div className="flex flex-wrap gap-2 mb-8">
            {userProfile.wantsTo?.map((item, index) => (
              <span
                key={index}
                className="bg-blue-100 text-blue-800 text-md font-medium px-3 py-1 rounded-full"
              >
                {item}
              </span>
            ))}
        ...
        <button
          ...
          className="mt-8 w-full bg-pink-400 hover:bg-pink-500 text-white font-bold py-3 rounded-full"
        >
          Send a Superlike
        </button>
      </section>
